...
print(os.path.isfile(f'./Simulations_R_varied/alive_list_' + str(extended_parameter_list)+'.txt'))
...
no_timestep = 1000
firstca = np.loadtxt(f'./Simulations_R_varied/alive_list_' + str(extended_parameter_list)+'.txt')
...
for radius in [1,2,3,4,5,6,7,8]:
    ...
    for k in range(N):
        ...
        alive_list = np.loadtxt(f'./Simulations_R_varied/alive_list_' + str(extended_parameter_list)+'.txt')
    ...
    plt.plot(meta_alive_list, label = "$R$ ="+str(radius))
