...
filepath = 'C:/Users/alaurits/Desktop/temp/simulations_era=10_R_varied_torus/torus_plain_200x200_alive_list_'
...
axins = zoomed_inset_axes(ax, 30, loc='upper left', 
    bbox_to_anchor=(-0.15,0.48,.2,.5), bbox_transform=ax.transAxes)


# eval_times = [20,100,1000]
# no_times = len(eval_times)
list_R = [1,2,3,4,5,6,7,8]
no_R = len(list_R)

meta_size = np.zeros((no_R,no_timestep, N))

land_area = 40000


for index_R in range(no_R):
    start_it = 1
    radius = list_R[index_R]
    for k in range(N):
        print(k)
        ...
        for i in range(0,no_timestep):
            meta_size[index_R, i, k] = land_area * 1/alive_list[i]
...
for index_R in range(no_R):
    ...
    ax.plot(list(range(1,no_timestep+1)),mean_size[index_R,:], label = "$R$ ="+str(radius))
    ax.fill_between(list(range(1,no_timestep+1)), errors[0,index_R, :], errors[1,index_R, :], alpha=0.2)

    axins.plot(list(range(no_timestep)),mean_size[index_R,:])
    axins.fill_between(list(range(no_timestep)), errors[0,index_R, :], errors[1,index_R, :], alpha=0.2)
